...
$backup = Join-Path $PSScriptRoot 'admin.css.before-stage1'

if (Test-Path $backup) {
  Copy-Item -Force $backup $css
  Write-Host 'Revert OK: admin.css.before-stage1 -> admin.css'
  exit 0
}
...
if ($content -notlike "*Mobile: nesting depth flattening*") {
  Write-Host 'Nichts zu revertieren - Stufe 1 ist nicht aktiv.'
  exit 0
}

$content = $content.Replace($block, "`n")
$content = $content.Replace('.project-days{padding:8px 6px 10px}', '.project-days{padding:12px}')
Set-Content -Path $css -Value $content -Encoding UTF8 -NoNewline
Write-Host 'Revert OK: Stufe-1-Block aus admin.css entfernt.'
