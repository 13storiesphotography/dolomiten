    ...
    async function handleLogin(){
      ...
      try{
        ...
        if(sessionData?.session?.user&&typeof rememberStudioForUser==='function'){
          await rememberStudioForUser(sessionData.session.user);
        }
        await boot();
      }catch(error){
    ...
    whatsNewBtn.addEventListener('click',toggleFeaturePanel);eventNameForm.addEventListener('submit',event=>{event.preventDefault();resolveEventName(eventNameInput.value)});eventNameCancel.addEventListener('click',()=>resolveEventName(null));addBtn.addEventListener('click',event=>{event.preventDefault();toggleAddMenu()});addMenu.querySelectorAll('[data-add-action]').forEach(button=>button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();runAddAction(button.dataset.addAction)}));document.addEventListener('pointerdown',event=>{if(addMenu.classList.contains('is-open')&&!event.target.closest('#addMenu')&&!event.target.closest('#addBtn'))setAddMenuOpen(false)});document.addEventListener('keydown',event=>{if(event.key==='Escape'){setAddMenuOpen(false);if(!eventNameDialog.classList.contains('hidden'))resolveEventName(null);if(typeof setIcsDialogOpen==='function'&&!$('icsImportDialog')?.classList.contains('hidden'))setIcsDialogOpen(false,{clearSession:true});if(typeof setProfileDialogOpen==='function'&&!$('profileDialog')?.classList.contains('hidden'))setProfileDialogOpen(false)}});searchInput.addEventListener('input',()=>{orderDirty=false;updateOrderButton();updateUnsavedBar();renderAll()});
    function renderLocationsGuarded(){if(!confirmUnsavedLocationRefresh())return;renderLocations()}
    locationSearchInput.addEventListener('input',renderLocationsGuarded);
    countryFilter.addEventListener('change',renderLocationsGuarded);
    regionFilter.addEventListener('change',renderLocationsGuarded);
    categoryFilter.addEventListener('change',renderLocationsGuarded);
    favoriteLocationsBtn.addEventListener('click',()=>{if(!confirmUnsavedLocationRefresh())return;locationFavoritesOnly=!locationFavoritesOnly;syncLocationFavoritesFilterBtn();renderLocations()});syncLocationFavoritesFilterBtn();if(typeof syncLocationSortSelect==='function')syncLocationSortSelect();if(typeof syncLocationGroupSelect==='function')syncLocationGroupSelect();locationSortSelect?.addEventListener('change',event=>{if(typeof setLocationSort==='function')setLocationSort(event.currentTarget.value)});locationGroupSelect?.addEventListener('change',event=>{if(typeof setLocationGroup==='function')setLocationGroup(event.currentTarget.value)});timelineTab.addEventListener('click',()=>setView('timeline'));calendarTab.addEventListener('click',()=>setView('calendar'));locationsTab.addEventListener('click',()=>setView('locations'));saveOpenBtn.addEventListener('click',async()=>{const keys=[...dirtyForms];for(const key of keys){const form=findFormByDirtyKey(key);if(!form)continue;if(isLocationDirtyKey(key))await saveLocationForm({preventDefault(){},currentTarget:form});else await saveForm({preventDefault(){},currentTarget:form})}});discardOpenBtn.addEventListener('click',discardOpenChanges);refreshDataBtn?.addEventListener('click',()=>refreshAdminData());window.addEventListener('beforeunload',()=>{if(typeof persistAdminDrafts==='function')persistAdminDrafts()});window.addEventListener('beforeunload',e=>{const icsOpen=typeof hasOpenIcsImport==='function'&&hasOpenIcsImport();if(!hasUnsavedChanges()&&!icsOpen)return;e.preventDefault();e.returnValue=''});document.querySelectorAll('[data-status-filter]').forEach(b=>b.addEventListener('click',()=>setStatusFilter(b.dataset.statusFilter)));$('projectFilter')?.addEventListener('change',event=>setProjectFilter(event.currentTarget.value));

    async function boot(){clearError();const {data,error}=await db.auth.getSession();if(error){setStatus('Session Fehler');showError('Session Fehler: '+error.message);return}const session=data.session;if(typeof isPasswordRecoveryActive==='function'&&isPasswordRecoveryActive()){if(typeof showPasswordRecoveryUI==='function')showPasswordRecoveryUI();return}loginBox.classList.toggle('hidden',!!session);if(typeof setLoginView==='function')setLoginView('login');const show=!!session;adminBox.classList.toggle('hidden',!show||currentView!=='timeline');calendarBox.classList.toggle('hidden',!show||currentView!=='calendar');toolbar.classList.toggle('hidden',!show||currentView==='locations');viewTabs.classList.toggle('hidden',!show);locationsBox.classList.toggle('hidden',!show||currentView!=='locations');whatsNewBtn.classList.toggle('hidden',!show);$('logoutBtn')?.classList.toggle('hidden',!show);setFeaturePanelOpen(false);setAddMenuOpen(false);addMenu.classList.add('hidden');addBtn.classList.toggle('hidden',!show);currentView=normalizeAdminView(currentView);viewTabs.classList.toggle('locations-active',currentView==='locations');viewTabs.classList.toggle('calendar-active',currentView==='calendar');timelineTab.classList.toggle('active',currentView==='timeline');calendarTab.classList.toggle('active',currentView==='calendar');locationsTab.classList.toggle('active',currentView==='locations');if(!session){clearAdminDraftSession();if(typeof renderLoginBranding==='function')renderLoginBranding();if(typeof syncAuthenticatedChrome==='function')syncAuthenticatedChrome(null);setStatus('');return}if(typeof rememberStudioForUser==='function')await rememberStudioForUser(session.user);if(typeof syncAuthenticatedChrome==='function')syncAuthenticatedChrome(session);renderWorkflowFilterButtons();const profile=typeof fetchUserProfile==='function'?await fetchUserProfile(session.user.id):null;const displayName=String(profile?.display_name||'').trim();setStatus(displayName?('Angemeldet als '+displayName):('Angemeldet als '+session.user.email));await loadShootings({skipConfirm:true});
    ...
    try{initSupabase();if(typeof initAuth==='function')initAuth();boot()}catch(error){setStatus('Shooting Planer konnte nicht starten');showError(error.message+' · Prüfe, ob der Supabase Script-Tag korrekt geladen wird.');console.error(error)}
